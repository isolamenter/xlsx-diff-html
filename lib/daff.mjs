...
// ── Shared HTML chrome ───────────────────────────────────────────────────────
// CSS common to both the single-page and side-by-side views. View-specific
// rules (row-number columns, side-by-side panes, cell-change colors) live in
// SINGLE_PAGE_CSS / SIDE_BY_SIDE_CSS below.
...
const SINGLE_PAGE_CSS = `td.modify {
...
// Shared dark header bar: title + "Next Change" jumper + 4 change-stat badges.
function renderHeaderBar(title, sheetName, stats) {
  ...
  <div style="display: flex; align-items: center; gap: 16px;">
    ...
    <div class="stats-panel">
      <div class="stat-badge stat-add" title="Added rows">
        ...
        <span class="stat-count">${stats.addedRowsCount}</span>
        <span class="stat-label">Added</span>
      </div>
      <div class="stat-badge stat-remove" title="Removed rows">
        <span class="stat-icon">-</span>
        <span class="stat-count">${stats.removedRowsCount}</span>
        <span class="stat-label">Removed</span>
      </div>
      <div class="stat-badge stat-modify" title="Modified rows">
        <span class="stat-icon">~</span>
        <span class="stat-count">${stats.modifiedRowsCount}</span>
        <span class="stat-label">Modified</span>
      </div>
      <div class="stat-badge stat-cells" title="Modified cells">
        <span class="stat-icon">✎</span>
        <span class="stat-count">${stats.modifiedCellsCount}</span>
...
export function csvDiffToHtml(oldCsv, newCsv, beforeLabel = 'Before', afterLabel = 'After', sheetName = 'Sheet') {
  ...
  const headerBar = renderHeaderBar('XLSX Single-Page Diff', sheetName, {
    addedRowsCount, removedRowsCount, modifiedRowsCount, modifiedCellsCount,
  });
...
export function csvDiffToHtmlSideBySide(oldCsv, newCsv, beforeLabel = 'Before', afterLabel = 'After', sheetName = 'Sheet') {
  ...
  function renderRows(rows, otherRows, rowNums, isLeft) {
    return rows.map((row, i) => {
      ...
      const cells = [];
      const rowNumHtml = row.type === 'empty' && rowNum === '' ? '&#160;' : esc(rowNum);

      // Leftmost row number cell (Excel style, sticky)
      if (isHeader) {
        cells.push(`<th class="row-num-header">${rowNumHtml}</th>`);
      } else {
        cells.push(`<td class="row-num">${rowNumHtml}</td>`);
      }
      ...
      for (let c = 0; c < numDiffCols; c++) {
        ...
        const cellHtml = row.type === 'empty' && val === '' ? '&#160;' : esc(val);
        cells.push(`<${tag}${cellCls}>${cellHtml}</${tag}>`);
      }
  ...
  const headerBar = renderHeaderBar('XLSX Side-by-Side Diff', sheetName, {
    addedRowsCount, removedRowsCount, modifiedRowsCount, modifiedCellsCount,
  });
